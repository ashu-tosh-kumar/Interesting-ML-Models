...
# i.e. At 6% coumpounded annually, principal will be doubled in 72/CAGR = 72/6 = 12 years.
...
# Solve for TimeYears: P(1+Interestrate/100)^TimeYears = 2*P
...
# test dataset
# Assuming general interst rates vary from 1% to 40% annually
...
# Now, instaed of comparing FinalLayerNode/InterestRate with TimeToDouble,
...
# Training model_nn
history = model_nn.fit(InterestRate,
                       InterestRate*TimeToDouble,
                       epochs=500,
                       batch_size=1000,
                       verbose=True)
...
outputs = [K.function([model_nn.input], [layer.output])([InterestRate, 1])
